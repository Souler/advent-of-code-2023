...
hands.forEach(hand => {
  if (hand.cards.includes('J')) {
    console.log(hand.cards.join(''), hand.type)
  }
})
...
function categorizeHandType(cards) {
  ...
  const jokerCount = groups.find(group => group.charAt(0) === 'J')?.length ?? 0

  const fiveOfAKind = groups.find(cards => cards.length >= (5 - jokerCount)) || jokerCount === 5
  if (fiveOfAKind) {
    return 'FIVE_OF_A_KIND'
  }

  const fourOfAKind = groups.find(cards => cards.length >= (4 - jokerCount))
  if (fourOfAKind) {
    return 'FOUR_OF_A_KIND'
  }
  
  const fullHouse = groups.length === 2 && jokerCount === 0 || groups.length === 3 && jokerCount > 0
  if (fullHouse) {
    return 'FULL_HOUSE'
  }

  const threeOfAKind = groups.find(cards => cards.length >= (3 - jokerCount))
  if (threeOfAKind) {
    return 'THREE_OF_A_KIND'
  }
  
  const pairs = groups.filter(group => group.length >= (2 - jokerCount))
  if (pairs.length == 2) {
    return 'TWO_PAIR'
  }
