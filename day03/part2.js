...
const result = schematic.reduce((acc, part) => {
  ...
  const partNumberNeighbors = neighbors.filter(neighbor => isNumeric(neighbor.value))

  if (partNumberNeighbors.length !== 2) {
    return acc
  }

  const [part1, part2] = partNumberNeighbors
  const gearRatio = Number.parseInt(part1.value) * Number.parseInt(part2.value)
